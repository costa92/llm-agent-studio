...
// 平台管理页（平台超级管理员专属，/platform）。三段：全局存储配置 / 全部组织 / 平台管理员。
// 入口由 whoami 网关（AppShell 仅对平台管理员渲染导航）；本页再以 whoami 做组件级门禁，
...
// 网关通过后的实际内容；拆出来让三段的 query 仅在确认是平台管理员后才发起。
function PlatformAdminContent() {
  return (
    <div className="flex flex-col gap-6 p-6">
      <header className="flex flex-col gap-1.5">
        <h1 className="font-heading text-[22px] font-bold text-text-1">平台管理</h1>
        <p className="text-[12px] text-text-3">
          服务端级设置：全局默认存储、全部组织一览、平台管理员管理。
        </p>
      </header>
