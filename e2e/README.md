...
llm-agent-studio 的首个 e2e 测试骨架，回放两个已在真实栈上验证通过的展示用例：

- **儿童绘本**（内置 picturebook 管线）：`case-picturebook.mjs`
...
| `MIN_IMAGES` | `1` | 音乐用例断言的分镜扇出图片数**下限**（实际张数由 LLM 生成的分镜数决定，故用下限而非精确值） |
...
# 完整绘本用例（真实生成，有 API 费用，约 10 分钟）——须显式开启
E2E_FULL=1 E2E_ORG=<org> node e2e/case-picturebook.mjs
...
E2E_SMOKE_ONLY=1 E2E_ORG=<org> node e2e/case-picturebook.mjs
```

也可用 `web/package.json` 里的封装脚本：`pnpm e2e:smoke` / `pnpm e2e:picturebook` / `pnpm e2e:music`。
...
### `case-picturebook.mjs`（`E2E_FULL=1`，**真实付费生成**）
create(kind=picturebook) → run{} → 轮询 `/state` 到 done → accept 每个 `pending_acceptance` 图片资产
→ 导出 pdf + epub → 断言导出字节数 > 0。绘本会产出 **图片和真实音频**（minimax speech-2.8-hd 现返回真实 MP3，
旧「骨架/占位」说法已过时）。
...
且 **无音频资产**。无音频是**载重断言**：非 picturebook 的 storyboard 扇出**只产图片**（真实音频是绘本专属路径）。
