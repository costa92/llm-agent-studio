...
// TestCreateExportNonPicturebook400: a standard project rejects export with 400.
func TestCreateExportNonPicturebook400(t *testing.T) {
	pool, db := openFreshExport(t)
	projID, _ := seedExportBook(t, pool, "org-std", "standard", 3, 3)
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("non-picturebook export: want 400, got %d (%s)", rec.Code, rec.Body.String())
	}
}
