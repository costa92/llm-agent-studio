...
// 双端 golden：镜像成品预览的宽松状态集——generating 等中间态被忽略，
// pending_acceptance/done/accepted 被纳入（工作流运行不走 HITL accept）。
func TestAssemble_NonExportableIgnored(t *testing.T) {
	assets := []Asset{
		{ID: "gen", ShotID: "s1", Type: "image", Status: "generating"},
		{ID: "await", ShotID: "s2", Type: "image", Status: "pending_acceptance"},
	}
	pages := Assemble("t", shotsFixture(), assets)
	if pages[1].ImageAssetID != "" {
		t.Errorf("generating ImageAssetID = %q, want empty (excluded)", pages[1].ImageAssetID)
	}
	if pages[2].ImageAssetID != "await" {
		t.Errorf("pending_acceptance ImageAssetID = %q, want \"await\" (included)", pages[2].ImageAssetID)
	}
}
...
// 宽松状态集：pending_acceptance image 现在计入（工作流运行不走 HITL accept）。
// 4 shots → contentCount=2 → 阈值 1；1 张 pending_acceptance image → 就绪。
func TestIsBookReady_PendingImageCounts(t *testing.T) {
	assets := []Asset{{ShotID: "s1", Type: "image", Status: "pending_acceptance"}}
	if !IsBookReady(shotsFixture(), assets) {
		t.Errorf("IsBookReady = false, want true (pending_acceptance now counts)")
	}
}

// generating 等中间态仍不计入。
func TestIsBookReady_GeneratingNotCounted(t *testing.T) {
	assets := []Asset{{ShotID: "s1", Type: "image", Status: "generating"}}
	if IsBookReady(shotsFixture(), assets) {
		t.Errorf("IsBookReady = true, want false (generating excluded)")
	}
}
...
func TestHasExportableImage(t *testing.T) {
	cases := []struct {
		name   string
		assets []Asset
		want   bool
	}{
		{"accepted", []Asset{{Type: "image", Status: "accepted"}}, true},
		{"pending_acceptance", []Asset{{Type: "image", Status: "pending_acceptance"}}, true},
		{"done", []Asset{{Type: "image", Status: "done"}}, true},
		{"generating", []Asset{{Type: "image", Status: "generating"}}, false},
		{"audio only", []Asset{{Type: "audio", Status: "accepted"}}, false},
		{"none", nil, false},
	}
	for _, c := range cases {
		if got := HasExportableImage(c.assets); got != c.want {
			t.Errorf("%s: HasExportableImage = %v, want %v", c.name, got, c.want)
		}
	}
}
