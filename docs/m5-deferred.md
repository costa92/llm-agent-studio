...
| **`todo_failed` 事件缺 `type` 字段** | 后端失败事件 payload 不带 `type`（与其他 todo_* 事件不一致），无法直接定位失败的是哪个 stage/pip | **前端 reducer 按 `todoId` 定位**（记录每个 todoId→stage/pip 映射，失败时回查），故失败节点不会"漂移"或滞留 running（见 commit `dcfdb6e`）。**记为后端 follow-up**：失败事件补 `type` 字段以与其他 todo 事件对齐，前端可移除 todoId 兜底。 |
...
| 项 | 现状 | M5 决策 |
|---|---|---|
| **视频/音频生成触发** | 后端 M4 已支持异步生成，但生成触发编排不在 M5 前端范围 | 前端只**展示/播放**库里返回的 `video`/`audio` 资产（`<video controls>` / `<audio controls>`），不新增生成触发 UI。 |
