...
> **✅ DONE** — 已实现并合并到 main(PR #57, `8a725f2`);任务全部完成,checkbox 于 2026-06-18 回填。
...
- [x] **Step 7: 写 store round-trip 测试(DB)**
...
- [x] **Step 1: 确认 script agent 输入/输出结构**
...
- [x] **Step 2: 写失败测试(绘本输入产出 characterSheet + 儿童 prompt)**
...
- [x] **Step 1: 写失败测试(绘本每页图+音,封面/空旁白只图)**
...
- [x] **Step 4: 跑测试确认通过 + 既有 worker 测试不回归**
...
- [x] **Step 4: 实现 RegenerateNarration**
...
- [x] **Step 4: 实现 PictureBookConfigForm**
...
- [x] **Step 6: 接入 EditProjectDialog + 创建表单**
...
- [x] **Step 3: PictureBookReader(失败测试)**
...
- [x] **Step 4: 跑确认失败 → 实现 PictureBookReader**
