...
**前端**
- `PictureBookReader`:pages 由 shots+assets 正确配对;封面/结尾页特殊渲染;翻页(上/下、环绕或到端停止)、页码、键盘;自动朗读模式音频 `ended` → 推进;缺音频降级;单页「重生成/编辑旁白」入口触发对应 mutation。
...
5. 后端:确认/小改 `runAsset` audio 路径接旁白文字/voice;**audio 计入 CostStore**。
...
9. 前端:音频解析/播放能力(`AssetAudio`)+ `PictureBookReader`(封面/结尾特殊渲染、自动朗读、单页重生成/编辑入口)+ 运行页入口。
