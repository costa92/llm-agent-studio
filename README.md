...
`JWT_SECRET` 与 `STUDIO_CONFIG_ENC_KEY` 是必填 secret，compose 不再内置弱默认值——
先 `cp .env.example .env` 并按其中注释生成随机值，否则 `docker compose up` 拒绝启动。
...
| 账号 | 密码 | 角色 | 说明 |
|---|---|---|---|
| `demo@studio.com` | `demo12345` | 普通用户（其组织的 org admin） | 走 组织→模型配置→项目→生成→审核→存储 全流程 |
| `pfadmin@s.com` | `pfpass1234` | **平台超级管理员** | 登录后见「平台」入口（全局存储 + 全部组织 + 平台管理员管理） |

- **自己造普通账号**：在 `/register` 用任意邮箱注册即自动登录、并可建组织。
- **自己造平台管理员**：把邮箱加入 studiod 的 `PLATFORM_ADMIN_EMAILS`（逗号分隔）后，用该邮箱注册即自动获授平台管理员（已注册用户在重启时种子化）。详见"平台管理员"。
...
- **workflow-only 转型（main，m23 / PR #149）**：产品从「内置绘本 + brief→LLM Planner 标准管线」收敛为**纯自定义节点工作流**——两条非工作流管线删除，DB 迁移 m23 把全部存量项目落 `kind='custom'`，「工作流」成为唯一执行模型（plan 由用户编排的 DAG 直接生成，无 LLM 规划步）。配套：工作流成品呈现（阅读器 / TTS 播放 / PDF·EPUB·ZIP 导出，PR #151–#153）+ 审核流重设计（PR #158）。详见 [docs/architecture/subsystem-map.md](docs/architecture/subsystem-map.md)。
...
- **TTS 暂以 `AsyncGenerator` 交付（I5 刻意分歧）**：spec §8.2/§4.2 规定短 TTS 走同步 `Generate`，但 M4 为保持异步引擎统一，TTS 骨架实现为 `AsyncGenerator`（submit/poll）。同步短 TTS 路径延后 M5（重启条件：M5 接真实 TTS HTTP 时按 spec 落同步 `Generate` 路径）——详见 [docs/m4-deferred.md](docs/m4-deferred.md)。**（已变更：MiniMax TTS 现为同步真实 `Generate`、显式不实现 `AsyncGenerator`，见 [docs/architecture/subsystem-map.md](docs/architecture/subsystem-map.md)。）**
- **regenerate video 的 duration 留 0**：重生成路径不精化时长（duration 精化延后 M5）；按秒计费在 regenerate 场景以估算/0 秒入账，poll-done 回填实际秒数。
- **真实 SaaS 适配器仅骨架**：Runway/Kling/Veo/TTS 适配器体内 HTTP 标 `// TODO(m5)`，未配 key → 不注册 → 返回「not configured」错误而非真 SaaS HTTP。真实接线 + 密钥 + idemKey→client-token header 映射延后 M5；M4 全程经 FakeAsync + loopback fetcher 在 sandbox 内活验（零外部 API）。**（已变更：必失败骨架适配器已随 Phase 2.1 下架，现 audio=MiniMax 同步真实、无真实 video 适配器，async 引擎经 FakeAsync 活验，见 [docs/architecture/subsystem-map.md](docs/architecture/subsystem-map.md)。）**
